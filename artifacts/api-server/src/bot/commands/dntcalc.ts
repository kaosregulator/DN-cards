...
// simultaneously without interfering. Item lookup uses the same MTTValues fuzzy
// search as /dnvaluecalculator.

const MAX_ITEMS = 3;
const PREFIX = "dntcalc";
...
function getSession(messageId: string, userId: string): UserSession {
  ...
  if (!s) {
    s = { yourItems: [], theirItems: [], lastSearch: null, touchedAt: Date.now() };
    sessions.set(key, s);
  } else {
...
export async function handlePostCalculator(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  const embed = new EmbedBuilder()
    ...
    .setFooter({ text: "Prices from mttvalues.com · each user has their own private session" });
...
// ── Button interactions ──────────────────────────────────────────────────────
export async function handleDntCalcButton(interaction: ButtonInteraction): Promise<void> {
  ...
  if (action === "pick") {
    ...
    const result = session.lastSearch?.[idx];
    if (!result) {
      await interaction.update({
...
async function handleCalculate(
  ...
  // Refresh latest MTTValues data before calculating.
  let items: DNItem[];
  try {
    items = await fetchItems();
  } catch {
    await interaction.editReply({
      content: "❌ Could not fetch latest DN prices. Please try again.",
    }).catch(() => {});
    return;
  }
...
// ── Modal submissions ──────────────────────────────────────────────────────────
export async function handleDntCalcModal(interaction: ModalSubmitInteraction): Promise<void> {
  ...
  try {
    ...
    await interaction.editReply({
      embeds: [buildPreview(session, "❌ Could not fetch DN items. Please try again.")],
      components: buildManageComponents(messageId, side, userId, items.length < MAX_ITEMS, items.length > 0),
    }).catch(() => {});
    return;
  }
  ...
  session.lastSearch = scored.map(s => s.i);
  const lines = scored.map((s, i) => {
    const rarity = s.i.rarity.map(rarityEmoji).join("") || "—";
    return `${i + 1}. ${rarity} **${s.i.name}** · 💎 ${formatValue(s.i)}`;
  }).join("\n");
