...
export interface LiveSiegeConfig {
  guildId: string;
  /** Shared target key used to prevent overlapping live/non-live sieges. */
  targetKey?: string;
...
export function isLiveSiegeTargetActive(targetKey: string): boolean {
...
export async function startLiveSiege(
  ...
  if (config.targetKey && activeTargetKeys.has(config.targetKey)) {
    await interaction.editReply({
      embeds: [new EmbedBuilder().setColor(0xc0392b)
        .setDescription("❌ That target is already under siege. Wait for the current assault to resolve.")],
      components: [],
    }).catch(() => {});
    return;
  }
  ...
  if (!posted) {
    ...
    if (session.targetKey) activeTargetKeys.delete(session.targetKey);
    return;
  }
