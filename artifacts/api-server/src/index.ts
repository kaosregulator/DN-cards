  ...
  await (async () => {
    ...
    try {
      ...
      const _dataMigrations: string[] = [
        ...
        "INSERT INTO cards(id,name,description,rarity,drop_weight,image_url,created_at,card_type,worth_value,burn_value,is_limited_edition,is_event_exclusive,max_copies,total_minted,flavor,droppable,in_packs,is_archived,podium_place) VALUES(318,'Patriot Event 1st Place','1st Place \u2014 evgeni2372006 \u2014 36 kills. The highest kill count of the event. Every Patriot launched brought them closer to victory.','epic',0,'/card-patriot-1st.png','2026-05-25T05:52:12.999Z','achievement',7500,3750,false,true,NULL,0,NULL,false,false,false,1) ON CONFLICT(id) DO UPDATE SET name=EXCLUDED.name,description=EXCLUDED.description,rarity=EXCLUDED.rarity,drop_weight=EXCLUDED.drop_weight,image_url=CASE WHEN EXCLUDED.image_url='' THEN cards.image_url ELSE EXCLUDED.image_url END,card_type=EXCLUDED.card_type,worth_value=EXCLUDED.worth_value,burn_value=EXCLUDED.burn_value,is_limited_edition=EXCLUDED.is_limited_edition,is_event_exclusive=EXCLUDED.is_event_exclusive,droppable=EXCLUDED.droppable,in_packs=EXCLUDED.in_packs,is_archived=EXCLUDED.is_archived;",
        "INSERT INTO cards(id,name,description,rarity,drop_weight,image_url,created_at,card_type,worth_value,burn_value,is_limited_edition,is_event_exclusive,max_copies,total_minted,flavor,droppable,in_packs,is_archived,podium_place) VALUES(319,'Patriot Event 2nd Place','2nd Place \u2014 tobiqwe123 \u2014 35 kills. One elimination short of glory, yet feared by every opponent.','epic',0,'/card-patriot-2nd.png','2026-05-25T05:52:12.999Z','achievement',5000,2500,false,true,NULL,0,NULL,false,false,false,2) ON CONFLICT(id) DO UPDATE SET name=EXCLUDED.name,description=EXCLUDED.description,rarity=EXCLUDED.rarity,drop_weight=EXCLUDED.drop_weight,image_url=CASE WHEN EXCLUDED.image_url='' THEN cards.image_url ELSE EXCLUDED.image_url END,card_type=EXCLUDED.card_type,worth_value=EXCLUDED.worth_value,burn_value=EXCLUDED.burn_value,is_limited_edition=EXCLUDED.is_limited_edition,is_event_exclusive=EXCLUDED.is_event_exclusive,droppable=EXCLUDED.droppable,in_packs=EXCLUDED.in_packs,is_archived=EXCLUDED.is_archived;",
