...
**Environment note:** The end-to-end siege harness requires the API boot migrations to have run first, because it writes the `hq_settings` table directly. A fresh or stale dev database can fail the harness until the API workflow is restarted once.